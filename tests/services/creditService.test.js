...
describe('creditService', () => {
  ...
  describe('completeFinancingStage', () => {
    it('should complete financing stage', async () => {
      mock
        .onPatch('/credit/bookings/1/financing/stage/1')
        .reply(200, { data: { completed: true } });
  ...
  describe('scheduleTitleTransfer', () => {
    it('should schedule title transfer', async () => {
      mock
        .onPatch('/credit/title-transfer/1/schedule')
        .reply(200, { data: { scheduled_date: '2026-03-01' } });
  ...
  describe('getConfirmedBookingById', () => {
    it('should fetch confirmed booking details', async () => {
      ...
      mock.onGet(`/credit/bookings/${bookingId}`).reply(200, createSuccessResponse(mockBooking));
    ...
    it('should handle 404 error', async () => {
      mock.onGet('/credit/bookings/999').reply(404, createErrorResponse('Not found', 404));
  ...
  describe('updateNegotiation', () => {
    it('should update negotiation status via PATCH', async () => {
      const bookingId = 1;
      const updateData = { status: 'agreed', notes: 'Customer accepted' };
      mock
        .onPatch(`/credit/bookings/negotiation/${bookingId}`)
        .reply(200, createSuccessResponse({ id: bookingId, ...updateData }));
  ...
  describe('processWaitingBooking', () => {
    it('should process waiting booking', async () => {
      ...
      mock
        .onPost(`/credit/bookings/waiting/${bookingId}/process`)
        .reply(200, createSuccessResponse({ id: bookingId, processed: true }));
  ...
  describe('getFinancingById', () => {
    it('should fetch financing details', async () => {
      ...
      mock
        .onGet(`/credit/financing/${financingId}`)
        .reply(200, createSuccessResponse(mockFinancing));
  ...
  describe('updateFinancing', () => {
    it('should update financing application', async () => {
      ...
      mock
        .onPut(`/credit/financing/${financingId}`)
        .reply(200, createSuccessResponse({ id: financingId, ...updateData }));
  ...
  describe('createTitleTransfer', () => {
    it('should create title transfer request', async () => {
      const bookingId = 100;
      mock
        .onPost(`/credit/bookings/${bookingId}/title-transfer`)
        .reply(201, createSuccessResponse({ id: 1, booking_id: bookingId }));

      const result = await creditService.createTitleTransfer({ booking_id: bookingId });
  ...
  describe('completeTitleTransfer', () => {
    it('should complete title transfer', async () => {
      ...
      mock
        .onPost(`/credit/title-transfer/${transferId}/complete`)
        .reply(200, createSuccessResponse({ id: transferId, completed: true }));
  ...
  describe('getSoldProjectById', () => {
    it('should fetch sold project details', async () => {
      ...
      mock
        .onGet(`/credit/sold-projects/${projectId}`)
        .reply(200, createSuccessResponse(mockProject));
  ...
  describe('Payment Plan (Tab 3.3)', () => {
    it('should get payment plan for booking', async () => {
      ...
      mock
        .onGet(`/credit/bookings/${bookingId}/payment-plan`)
        .reply(200, createSuccessResponse(mockPlan));
    ...
    it('should create payment plan for booking', async () => {
      const bookingId = 1;
      const data = {
        installments: [{ due_date: '2026-03-01', amount: 100000, description: 'الدفعة الأولى' }],
      };
      mock
        .onPost(`/credit/bookings/${bookingId}/payment-plan`)
        .reply(201, createSuccessResponse({ id: 1, ...data }));
    ...
    it('should update installment', async () => {
      ...
      mock
        .onPut(`/credit/payment-installments/${installmentId}`)
        .reply(200, createSuccessResponse({ id: installmentId, ...data }));
    ...
    it('should delete installment', async () => {
      const installmentId = 1;
      mock
        .onDelete(`/credit/payment-installments/${installmentId}`)
        .reply(200, createSuccessResponse({ deleted: true }));
  ...
  describe('submitClaim', () => {
    it('should submit claim file', async () => {
      const claimId = 1;
      mock
        .onPost(`/credit/claim-files/${claimId}/submit`)
        .reply(200, createSuccessResponse({ id: claimId, submitted: true }));
  ...
  describe('approveClaim', () => {
    it('should approve claim payment', async () => {
      ...
      mock
        .onPost(`/credit/claim-files/${claimId}/approve`)
        .reply(200, createSuccessResponse({ id: claimId, approved: true }));
  ...
  describe('getClaimFileCandidates', () => {
    it('should fetch candidates (sold bookings without claim files)', async () => {
      ...
      mock
        ...
        .reply(200, { data: mockCandidates, meta: { total: 1 } });

      const result = await creditService.getClaimFileCandidates({ per_page: 15 });
  ...
  describe('generateBulkClaimFiles', () => {
    it('should generate individual claim files for multiple reservations', async () => {
      ...
      expect(sentBody.reservation_ids).toEqual([123, 124, 125]);
      expect(result).toBeDefined();
  ...
  describe('createCombinedClaimFile', () => {
    it('should create combined claim file with multiple booking ids', async () => {
      const payload = { booking_ids: [10, 20, 30], claim_type: 'commission', notes: 'test' };
      mock
        .onPost('/credit/claim-files/combined')
        .reply(201, { data: { id: 99, claim_amount: 450000, booking_ids: [10, 20, 30] } });

      const result = await creditService.createCombinedClaimFile(payload);

      expect(mock.history.post.length).toBe(1);
      const sentBody = JSON.parse(mock.history.post[0].data);
      expect(sentBody.booking_ids).toEqual([10, 20, 30]);
      expect(sentBody.claim_type).toBe('commission');
      expect(result).toBeDefined();
  ...
  describe('Error Handling', () => {
    ...
    it('should handle 404 Not Found', async () => {
      mock
        .onGet('/credit/bookings/confirmed/999')
        .reply(404, createErrorResponse('Not found', 404));
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle invalid booking ID', async () => {
      mock
        .onGet('/credit/bookings/confirmed/invalid')
    ...
    it('should handle negative booking ID', async () => {
      mock
        .onGet('/credit/bookings/confirmed/-1')
        .reply(400, createErrorResponse('Invalid ID', 400));
