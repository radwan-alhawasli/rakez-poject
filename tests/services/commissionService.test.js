...
describe('commissionService', () => {
  ...
  describe('getCommissions', () => {
    it('should fetch list of commissions', async () => {
      ...
      mock.onGet('/commissions').reply(200, { data: mockCommissions })
  ...
  describe('createCommission', () => {
    it('should create a new commission', async () => {
      ...
      mock.onPost('/commissions').reply(201, { data: { id: 1 } })
  ...
  describe('calculateCommission', () => {
    it('should calculate commission', async () => {
      ...
      mock.onPost('/commissions/calculate').reply(200, { data: mockCalculation })
  ...
  describe('getDeposits', () => {
    it('should fetch list of deposits', async () => {
      ...
      mock.onGet('/deposits').reply(200, { data: mockDeposits })
  ...
  describe('createDeposit', () => {
    it('should create a new deposit', async () => {
      ...
      mock.onPost('/deposits').reply(201, { data: { id: 1 } })
  ...
  describe('processDeposit', () => {
    it('should process a deposit payment', async () => {
      ...
      mock.onPost(`/deposits/${depositId}/process`).reply(200, { data: { processed: true } })
  ...
  describe('Error Handling', () => {
    it('should handle 400 Bad Request for getCommissions', async () => {
      mock.onGet('/commissions').reply(400, { error: 'Bad request' })
    ...
    it('should handle 401 Unauthorized for createCommission', async () => {
      mock.onPost('/commissions').reply(401, { error: 'Unauthorized' })
    ...
    it('should handle 403 Forbidden for calculateCommission', async () => {
      mock.onPost('/commissions/calculate').reply(403, { error: 'Forbidden' })
    ...
    it('should handle 404 Not Found for processDeposit', async () => {
      mock.onPost('/deposits/999/process').reply(404, { error: 'Not found' })
    ...
    it('should handle 422 Validation Error for createCommission', async () => {
      mock.onPost('/commissions').reply(422, { error: 'Validation failed', errors: { contract_id: ['Required'] } })

      await expect(commissionService.createCommission({})).rejects.toThrow()
    })

    it('should handle 500 Server Error for getDeposits', async () => {
      mock.onGet('/deposits').reply(500, { error: 'Server error' })

      await expect(commissionService.getDeposits()).rejects.toThrow()
    })

    it('should handle network errors', async () => {
      mock.onGet('/commissions').networkError()

      await expect(commissionService.getCommissions()).rejects.toThrow()
    })

    it('should handle timeout errors', async () => {
      mock.onPost('/commissions/calculate').timeout()
  ...
  describe('Edge Cases', () => {
    it('should handle null response data', async () => {
      mock.onGet('/commissions').reply(200, { data: null })
    ...
    it('should handle empty array response for getCommissions', async () => {
      mock.onGet('/commissions').reply(200, { data: [] })
    ...
    it('should handle invalid deposit ID', async () => {
      mock.onPost('/deposits/invalid/process').reply(400, { error: 'Invalid ID' })

      await expect(commissionService.processDeposit('invalid', {})).rejects.toThrow()
    })

    it('should handle empty commission data object', async () => {
      mock.onPost('/commissions').reply(422, { error: 'Missing required fields' })

      await expect(commissionService.createCommission({})).rejects.toThrow()
    })

    it('should handle zero amount', async () => {
      mock.onPost('/commissions').reply(422, { error: 'Amount must be greater than zero' })
