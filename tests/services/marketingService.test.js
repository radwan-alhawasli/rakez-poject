...
describe('marketingService', () => {
  ...
  it('updates task status using PATCH /marketing/tasks/:id/status', async () => {
    mock.onPatch('/marketing/tasks/7/status').reply(200, { data: { id: 7, status: 'completed' } });
  ...
  it('getDeveloperPlan uses /marketing/developer-plans/:contractId', async () => {
    mock.onGet('/marketing/developer-plans/42').reply(200, { data: { id: 1, contract_id: 42 } });
    const result = await marketingService.getDeveloperPlan(42);
    expect(mock.history.get[0].url).toBe('/marketing/developer-plans/42');
    expect(result.contract_id).toBe(42);
  });

  it('storeDeveloperPlan posts to /marketing/developer-plans', async () => {
    mock.onPost('/marketing/developer-plans').reply(201, { data: { id: 1 } });
    const result = await marketingService.storeDeveloperPlan({ contract_id: 42 });
    expect(mock.history.post[0].url).toBe('/marketing/developer-plans');
    expect(result.id).toBe(1);
  });

  it('getEmployeePlans uses /marketing/employee-plans', async () => {
    mock.onGet('/marketing/employee-plans').reply(200, {
      data: [{ id: 1 }],
      meta: { pagination: { total: 1 } },
    });
    const result = await marketingService.getEmployeePlans(null, { per_page: 10 });
    expect(mock.history.get[0].url).toContain('/marketing/employee-plans');
    expect(result.items).toBeDefined();
  });

  it('createEmployeePlan posts to /marketing/employee-plans', async () => {
    mock.onPost('/marketing/employee-plans').reply(201, { data: { id: 1 } });
    const result = await marketingService.createEmployeePlan({ project_id: 5 });
    expect(mock.history.post[0].url).toBe('/marketing/employee-plans');
    expect(result.id).toBe(1);
  });
});
