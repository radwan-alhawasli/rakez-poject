...
describe('authService', () => {
  ...
  describe('login', () => {
    it('should login and store token and user when API returns data with token', async () => {
      ...
      expect(mockPost).toHaveBeenCalledWith('/login', { email: 'admin@test.com', password: 'password' })
    ...
    it('should store refresh_token when provided', async () => {
      mockPost.mockResolvedValue({
        data: { data: { token: 't', refresh_token: 'rt', user: { name: 'U', email: 'u@t.com', type: 1 } } }
      })
    ...
    it('should preserve permissions from API when provided', async () => {
      ...
      mockPost.mockResolvedValue({
        data: { data: { token: 't', user: { name: 'U', email: 'u@t.com', type: 1, permissions: perms } } }
      })

      const result = await authService.login('u@t.com', 'p')

      expect(secureStorage.setUserInfo).toHaveBeenCalledWith(expect.objectContaining({ permissions: perms }))
