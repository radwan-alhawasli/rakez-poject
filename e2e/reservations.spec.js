...
test.describe('Reservations Page', () => {
  ...
  test('should load the reservations page at /reservations', async ({ page }) => {
    ...
    await expect(page.getByTestId('e2e-welcome-title')).toContainText('حجوزاتي');
  });
  ...
  test('should have page subtitle describing purpose', async ({ page }) => {
    await page.goto('/reservations');
    await expect(page.getByTestId('e2e-welcome-subtitle')).toContainText('عرض جميع الوحدات التي قمت بحجزها');
  });
