  ...
  <div class="project-tracker-view">
    ...
    <template v-else-if="project">
      ...
      <div class="tracker-container">
        ...
        <!-- UNITS TAB (design match: header + 3 buttons + unit cards grid) -->
            ...
            <div v-for="unit in units" :key="unit.id" class="unit-card">
              ...
              <div class="unit-specs">
                <span class="unit-spec"
                  ...
                  {{ unit.bedrooms || unit.rooms || 3 }}</span
                >
                <span class="unit-spec"
                  ><svg
                    viewBox="0 0 24 24"
                ...
                <span class="unit-spec"
                  ...
                  {{
                    unit.floor != null && !Number.isNaN(Number(unit.floor))
                      ? 'الدور ' + unit.floor
                      : '—'
                  }}</span
                >
...
export default {
  ...
  setup() {
    ...
    // 7 stages (design match to reference images)
    const stages = reactive([
      {
        name: 'الصكوك و الرخصه',
        status: 'pending',
      ...
      {
        name: 'شهادة اتمام و اخرى',
        status: 'pending',
        apiKey: 'completion_certificate_url',
        value: '',
        entryDate: '',
        completedAt: null,
      },
        ...
        try {
          if (isEditor) {
            ...
            project.value = {
              ...
              advertiser_number:
                raw.advertiser_number || raw.advertiser_section_url || raw.advertiser_num_id || '—',
      ...
      confirmModalConfig.value = {
        ...
        resolve: async () => {
          try {
            ...
            toast.error(
              'حدث خطأ أثناء القبول: ' + (error.response?.data?.message || error.message)
            );
          }
        },
      };
    ...
    const downloadContractForProject = async () => {
      ...
      try {
        if (typeof contractService.downloadContract === 'function') {
          await contractService.downloadContract(project.value.id);
