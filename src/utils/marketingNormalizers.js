...
export const normalizeProjectDetails = (raw = {}) => {
  ...
  const averageUnitPrice = units.length
    ...
    : toNumber(raw.average_unit_price || raw.avg_price)

  return {
    ...raw,
    units,
    available_units_count: toNumber(raw.available_units_count, availableUnits.length),
    pending_units_count: toNumber(raw.pending_units_count, pendingUnits.length),
    available_units_value: toNumber(raw.available_units_value, availableUnitsValue),
    average_unit_price: toNumber(raw.average_unit_price, averageUnitPrice),
    commission_percentage: toNumber(raw.commission_percentage)
  }
}
...
export const normalizeExpectedSale = (raw = {}) => {
  ...
  const handRaise = toNumber(
    raw.hand_raises ??
    raw.raised_hands ??
    raw.hand_raise_count
  )

  const conversionRateRaw = toNumber(raw.conversion_rate, 1)
  const conversionRatePercent = conversionRateRaw <= 1 ? conversionRateRaw * 100 : conversionRateRaw

  const expectedBookings =
    toNumber(raw.expected_bookings) || Math.round((direct + handRaise) * (conversionRatePercent / 100))

  const campaignBudget = toNumber(
    raw.campaign_budget ??
    raw.marketing_budget ??
    raw.marketing_value ??
    raw.total_budget
  )

  const expectedBookingValue = toNumber(
    raw.expected_booking_value ??
    raw.total_expected_booking_value ??
    raw.average_booking_value
  )
  ...
  return {
    ...
    deposit_per_booking: expectedBookings > 0
