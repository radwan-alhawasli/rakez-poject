...
export function handleServiceError(error, operation, method = 'get', defaultValue = []) {
  ...
  if (method.toLowerCase() === 'get' && (status === 403 || status === 404)) {
    ...
    if (appConfig.isDevelopment) {
      logger.debug(`${operation} - ${status === 403 ? 'Forbidden' : 'Not Found'}:`, error?.response?.data?.message || error?.message)
    }
  ...
  if (status !== 403 && status !== 404) {
    ...
    logger.warn(`${operation} - ${status === 403 ? 'Forbidden' : 'Not Found'}:`, error?.response?.data?.message || error?.message)
  }
