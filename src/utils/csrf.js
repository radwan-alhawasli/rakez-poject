  ...
  tokenFetchPromise = csrfRequestClient
    ...
    .catch(error => {
      ...
      if (error.response && error.response.status !== 404) {
        ...
        logger.error(
          `Failed to fetch CSRF token: ${error.response.status} ${error.response.statusText}`
        );
...
export function setupCsrfInterceptor(client = null) {
  ...
  if (!clientInstance) {
    throw new Error(
      'CSRF utility not initialized. Call initCsrf() or pass apiClient as parameter.'
    );
  }
