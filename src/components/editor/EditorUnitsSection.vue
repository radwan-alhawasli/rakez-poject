      ...
      <table class="metrics-table table-mobile-stacked">
        ...
        <tbody>
          <tr v-for="unit in contractUnits" :key="unit.id">
            ...
            <td data-label="النوع">{{ unit.type || unit.unit_type || '—' }}</td>
            <td data-label="الحالة">{{ unit.status || '—' }}</td>
          </tr>
          <tr v-if="contractUnits.length === 0">
            <td data-label="" colspan="3" style="text-align: center; padding: 40px; color: var(--color-dark-gray)">
...
defineProps({
  ...
  contractUnits: { type: Array, default: () => [] },
