    ...
    <div v-if="unit" class="detail-body">
      ...
      <div class="unit-info-bar">
        <div class="unit-info-item">
          ...
          <span class="unit-info-value">{{ unit.unit_number || unit.reservation_id || unit.id }} / {{ unit.project_name || 'غير محدد' }}</span>
        </div>
        <div class="unit-info-item">
          <span class="unit-info-label">العميل:</span>
          <span class="unit-info-value">{{ unit.customer_name || unit.client_name || 'غير محدد' }}</span>
        </div>
      </div>
      ...
      <div class="unit-meta-row">
        ...
        <div class="editable-fields">
          <div class="field-group">
            ...
            <input v-model.number="commissionForm.commission_percentage" type="number" class="form-input input-sm" min="0" max="100" step="0.1" />
          </div>
          <div class="field-group">
        ...
        <form @submit.prevent="handleCreateCommission" class="create-form">
          <div class="form-row">
            <div class="form-group">
              ...
              <input v-model.number="commissionForm.final_selling_price" type="number" class="form-input" min="0" required />
            </div>
            <div class="form-group">
              <label>نسبة العمولة (%)</label>
              <input v-model.number="commissionForm.commission_percentage" type="number" class="form-input" min="0" max="100" required />
            </div>
            <div class="form-group">
              <label>الفريق المسؤول</label>
              <input v-model="commissionForm.team_responsible" type="text" class="form-input" placeholder="فريق المبيعات" />
            </div>
          </div>
          <div class="form-row">
            <div class="form-group">
              <label>مصاريف التسويق (ر.س)</label>
              <input v-model.number="commissionForm.marketing_expenses" type="number" class="form-input" min="0" />
            </div>
            <div class="form-group">
              <label>رسوم البنك (ر.س)</label>
              <input v-model.number="commissionForm.bank_fees" type="number" class="form-input" min="0" />
            </div>
          </div>
        ...
        <div class="distribution-grid">
          ...
          <div class="distribution-panel marketing-panel">
            ...
            <div class="marketer-section">
              ...
              <div v-for="(row, idx) in leadGenRows" :key="'lg-' + idx" class="distribution-row">
                ...
                <input v-model.number="row.percentage" type="number" class="form-input input-pct" min="0" max="100" placeholder="النسبة %" />
          ...
          <div class="distribution-panel management-panel">
            ...
            <div v-if="hasExternalBroker" class="external-fields">
              ...
              <div class="form-group">
                <label>النسبة %</label>
                <input v-model.number="externalMarketer.percentage" type="number" class="form-input input-sm" min="0" max="100" />
              </div>
            </div>
            <div class="management-percentages">
              <div class="mgmt-row">
                <label>قائد الفريق</label>
                <input v-model.number="managementPct.team_leader" type="number" class="form-input input-pct" min="0" max="100" />
              </div>
              <div class="mgmt-row">
                <label>مساعد مدير مشروع</label>
                <input v-model.number="managementPct.assistant_pm" type="number" class="form-input input-pct" min="0" max="100" />
              </div>
              <div class="mgmt-row">
                <label>مدير مشروع</label>
                <input v-model.number="managementPct.project_manager" type="number" class="form-input input-pct" min="0" max="100" />
              </div>
              <div class="mgmt-row">
                <label>المالك</label>
                <input v-model.number="managementPct.owner" type="number" class="form-input input-pct" min="0" max="100" />
              </div>
              <div class="mgmt-row">
                <label>مدير المبيعات</label>
                <input v-model.number="managementPct.sales_manager" type="number" class="form-input input-pct" min="0" max="100" />
              </div>
              <div class="mgmt-row">
                <label>قسم المشاريع</label>
                <input v-model.number="managementPct.projects_department" type="number" class="form-input input-pct" min="0" max="100" />
              </div>
              <div class="mgmt-row">
                <label>الإدارة</label>
                <input v-model.number="managementPct.management" type="number" class="form-input input-pct" min="0" max="100" />
              </div>
              <div class="mgmt-row">
                <label>CEO</label>
                <input v-model.number="managementPct.ceo" type="number" class="form-input input-pct" min="0" max="100" />
              </div>
            </div>
            <button type="button" class="btn-primary btn-save" @click="handleSaveDistributions" :disabled="isSaving">حفظ التوزيعات</button>
          </div>
        </div>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const finalPrice = computed(() => props.unit?.final_sale_price ?? props.unit?.total_value ?? commissionForm.final_selling_price ?? 0)
    const commissionStatus = computed(() => props.unit?.commission_status || props.unit?.status || (commissionSummary.value ? 'Approved' : null))
    ...
    const companyAmount = computed(() => {
      ...
      const distTotal = distributions.value.reduce((sum, d) => sum + (parseFloat(d.amount) || 0), 0)
      return Math.max(0, net - distTotal)
    })

    const formatCurrency = (val) => {
      if (!val) return '0 ر.س'
      return new Intl.NumberFormat('en-US', { style: 'currency', currency: 'SAR', maximumFractionDigits: 2 }).format(val)
    }
      ...
      try {
        ...
        if (!hasPopulatedFromApi.value && distributions.value.length > 0) {
          ...
          distributions.value.forEach(d => {
            ...
            else if (type === 'team_leader') managementPct.team_leader = parseFloat(d.percentage) || 0
            else if (type === 'assistant_pm') managementPct.assistant_pm = parseFloat(d.percentage) || 0
            else if (type === 'project_manager') managementPct.project_manager = parseFloat(d.percentage) || 0
            else if (type === 'sales_manager') managementPct.sales_manager = parseFloat(d.percentage) || 0
            else if (type === 'owner') managementPct.owner = parseFloat(d.percentage) || 0
            else if (type === 'projects_department') managementPct.projects_department = parseFloat(d.percentage) || 0
            else if (type === 'management') managementPct.management = parseFloat(d.percentage) || 0
    ...
    const initFormFromUnit = () => {
      ...
      commissionForm.contract_unit_id = props.unit.contract_unit_id || props.unit.unit_id || props.unit.id
      commissionForm.final_selling_price = props.unit.final_sale_price ?? props.unit.total_value ?? 0
    ...
    const buildDistributionsPayload = () => {
      ...
      leadGenRows.forEach(r => {
        if (r.user_id && r.percentage > 0) dists.push({ type: 'lead_generation', user_id: r.user_id, percentage: r.percentage })
      })
      persuasionRows.forEach(r => {
        if (r.user_id && r.percentage > 0) dists.push({ type: 'persuasion', user_id: r.user_id, percentage: r.percentage })
      })
      closingRows.forEach(r => {
        if (r.user_id && r.percentage > 0) dists.push({ type: 'closing', user_id: r.user_id, percentage: r.percentage })
      })
      if (managementPct.team_leader > 0) dists.push({ type: 'team_leader', user_id: null, percentage: managementPct.team_leader })
      if (managementPct.assistant_pm > 0) dists.push({ type: 'assistant_pm', user_id: null, percentage: managementPct.assistant_pm })
      if (managementPct.project_manager > 0) dists.push({ type: 'project_manager', user_id: null, percentage: managementPct.project_manager })
      if (managementPct.owner > 0) dists.push({ type: 'owner', user_id: null, percentage: managementPct.owner })
      if (managementPct.sales_manager > 0) dists.push({ type: 'sales_manager', user_id: null, percentage: managementPct.sales_manager })
      if (managementPct.projects_department > 0) dists.push({ type: 'projects_department', user_id: null, percentage: managementPct.projects_department })
      if (managementPct.management > 0) dists.push({ type: 'management', user_id: null, percentage: managementPct.management })
      if (managementPct.ceo > 0) dists.push({ type: 'ceo', user_id: null, percentage: managementPct.ceo })
