<template>
  <Dialog :open="open" @update:open="onOpenChange">
    <AppModalContent :class="contentClass" :size="size" :hide-close="hideClose">
      <div class="app-modal-inner flex flex-col max-h-[85vh]" dir="rtl">
        <!-- Header: title + optional subtitle (from prop or slot) -->
        <div
          v-if="title || $slots.header || $slots.title"
          class="app-modal-header flex flex-row items-center justify-between gap-4 border-b border-[var(--color-light-gray)] pb-4 pt-6 px-6"
        >
        ...
        <!-- Body -->
        <div
          class="app-modal-body flex-1 overflow-y-auto px-6 pb-6 pt-6"
        ...
        <div
          v-if="$slots.footer"
          class="app-modal-footer flex flex-row flex-wrap items-center justify-end gap-3 border-t border-[var(--color-light-gray)] px-6 py-4"
        >
          <slot name="footer" />
        </div>
      </div>
