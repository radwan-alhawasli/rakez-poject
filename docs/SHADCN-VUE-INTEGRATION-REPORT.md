...
- **src/components/ui/Card.vue:** New. Wrapper that applies `card-luxury`. Props: `class`. Slot for content. No existing cards were replaced.

All three are design-preserving wrappers: they rely on current global luxury CSS and do not introduce new colors, fonts, radius, or spacing. Existing `.btn-primary`, `.form-input`, `.card-luxury`, etc. are unchanged and still used across the app.
...
- **Adopt UI components gradually:** Where convenient, start using `@/components/ui/Button`, `Input`, and `Card` in new or refactored code; keep existing class-based markup until each usage is migrated and verified.
...
### Phase 2 — Migration to UiButton / UiInput / UiCard / UiSelect
...
Other views and modals can follow the same pattern: import `Button`, `Input`, `Card`, or `Select` from `@/components/ui/*`, replace `<button class="btn-primary">` with `<Button>`, `<input class="form-input">` with `<Input v-model="...">`, `<select class="form-input">` with `<Select v-model="...">` (options in default slot), and `<div class="card-luxury">` with `<Card>` where desired. No visual or behavior change beyond a single component API.
